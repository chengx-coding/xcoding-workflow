...
- 在实现与验证之后，改动了可分析代码的工单会产出一份**变更报告**：一个离线 HTML 文件，逐条说明改了什么、为什么存在、背后的设计与权衡，以及它处于更大流程的哪一环，并附上机械证明，说明没有可分析变更单元被漏掉。报告是**挂载它的那些生命周期**中的一个阶段——普通受管工单、自适应受管工单，以及新建受管功能——对其处置表中列出的其余每个生命周期入口，[覆盖协议](skills/xc-change-report/references/coverage-protocol.md)都记录了处置，而不是声明尚未记录的覆盖。
