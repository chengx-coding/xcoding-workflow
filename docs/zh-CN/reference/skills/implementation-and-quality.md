...
- **用途：** 把单个工单的变更集合产出为一个离线自包含 HTML 报告，以及一份覆盖清单：清单证明每个可分析变更单元都有分析覆盖，分析与所引用代码绑定，并说明改了什么、为什么存在、体现了什么设计、处于更大流程的哪一环。
- **公开入口：** 必填 `report_path`、`manifest_path`、`repo_path`、`baseline_commit`、`language`、`strength`；校验还必填 `stage`，`verdicts_path` 只在 final 阶段必填；`gate_required` 默认 `false`。
- **典型用法：** 枚举变更集合并发布清单，生成骨架，为清单中的每个单元写入八个必需的分析字段（多个单元可以共用一段分析，只要该段声明覆盖它们的全部锚点），然后在准确性复核前后分别以 `stage=coverage` 与 `stage=final` 校验。每一轮都重新生成同一个报告；代码返工导致报告过期时走刷新，而不是判为节点失败，且刷新循环有界。
