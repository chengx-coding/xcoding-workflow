...
[English](../../development/documentation-maintenance.md)

## 语言契约

英文 README 和文档页面是规范版本。`docs/` 下每个英文页面都必须在 `docs/zh-CN/` 下具有相同相对路径的简体中文镜像。镜像必须保持相同主题、事实边界、导航和实质信息，但无需逐行直译。每对页面都必须提供正确的双向语言链接。
