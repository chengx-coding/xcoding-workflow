...
- **Public entry:** the required package exposes lifecycle, query, and recovery operations as `xcoding runtime <command> ...`. Opt-in completion adds repeated `--check-result-json`; opt-in gates add `--gate-outcome` and `--decision`. Every parameter that carries JSON or `key=value` text also has a file form — `--check-result-file`, `--set-file`, and `--metadata-file` — for callers whose shell rewrites quoting. `close-group` and `reopen-group` accept the group as either `--group` or `--node`.
...
Application Service, complete command specification, typed query facade, and
default template. `src/xcoding/viewer/` owns Viewer server, picker, lifecycle,
and static resources. `src/xcoding/daemon/` owns the authenticated read-only
daemon.

The matching package is required. The runtime Skill contains no runtime or
Viewer fallback. Its sole `scripts/orchestration.py` file is a deprecated thin
adapter that replaces itself with `xcoding runtime`; a missing executable
returns `xcoding_unavailable`.
...
default transport switch. `xcoding runtime` remains a direct local command and
does not discover or start the daemon.
...
- **Purpose:** provide a script-free facade over the package-owned, loopback-only, read-only viewer.
- **Public entry:** launch `xcoding viewer --tree <tree-ref>`; use `--allow-root` only for additional permitted directories.
