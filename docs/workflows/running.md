...
2. **Open and initialize the work order.** Create the durable workbench, initialize its managed tree, and fix the work order document language before writing the first top-level document. Use the `tmp_path` the opener returns for temporary and process files, and create `<workbench>/tmp/` when an existing workbench does not have it. Then capture the baseline while the worktree is still at its opened state, because the report is later proven against it and the opened state cannot be re-materialised afterwards: run the capture form of `skills/xc-change-report/scripts/capture_baseline.py` from the [`xc-change-report`](../../skills/xc-change-report/SKILL.md) package. It mirrors every tracked path into `<workbench>/tmp/baseline-worktree/` and every untracked path into `<workbench>/tmp/baseline-untracked/` - both directories are selectable with `--baseline-worktree-dir` and `--baseline-untracked-dir` - and records the two snapshot paths with the worktree digest in `<workbench>/tmp/baseline-open-state.json`. A later `--verify-existing` run recomputes that digest from the recorded worktree snapshot, re-verifies the recorded counts against git and the two recorded snapshots, and fails closed on any mismatch, so the digest published at open is never rewritten.
...
9. **Explain the change.** [`xc-change-report`](../../skills/xc-change-report/SKILL.md) turns the work order's change set into one self-contained offline HTML report plus the coverage manifest it is validated against. Every analysable change unit is covered by an analysis, bound to the code it cites, of what changed, why it exists, the design it embodies, and its place in the larger flow. The report is a stage of the lifecycles that mount it, and this full lifecycle is one of them: here the report group is the last group before the result document, mounted after the verification group, and this lifecycle mounts no top-level review node, so a review a bridge or an approved solution requires is extra work inside the implementation path and happens before the report rather than between the report and the result document. Code rework makes a report stale and refreshes the same single report rather than failing a node. A read-only mode produces no report, and a measured count of zero analysable change units is recorded as a skip decision. The report's human gate is optional and closed by default.
10. **Review when required.** Where independent quality assessment is required, [`xc-review`](../../skills/xc-review/SKILL.md) evaluates immutable inputs and produces traceable findings. On the full lifecycle such a review is extra work inside the implementation path, so it happens before the report and never between the report and the result document; on the adaptive path the main session places the report before the review leaf, so the reviewer reads the report. A review that requires code rework refreshes the report instead of leaving a stale one in place.
