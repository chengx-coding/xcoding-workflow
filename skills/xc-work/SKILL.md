...
A mutation-mode plan always carries the `change_report` capability, because a work order that produces analyzable change units must produce a report; read-only modes never acquire it, even when unknown facts escalate to the full capability set. Embed the `xc-change-report` subtree once after the implementation and verification nodes, or run one `role=report` node carrying the same contract; both forms produce exactly one report. The plan receipt's `report_strength` is the initial strength tier derived from the confirmed `risk` and `audit` facts, and the manifest measurement may still upgrade it upward, never downward. When the measured analyzable change unit count is zero, the report is not produced and the main session records `work_order.report_skip_reason=zero_analyzable_units` instead.
...
9. Add approved implementation nodes through `xc-implementation`, then verification nodes through `xc-verification` only after `approved-solution-continuation` is selected. Publish each node's declared source IDs through its node-specific blackboard key before requesting its control packet; do not substitute ancestor or group IDs.
   A work order that produces analyzable change units also produces a change report. `report-group` sits between `verification-group` and `result-document`, because only implemented and verified code has a state worth describing. When the adaptive path selects `independent_review`, or the project bridge adds a review step, produce the report before that review: the report is the reviewer's input, and a review that changes code refreshes it once more afterwards.
