...
1. Read `AGENTS.md`, `.xcoding/WORKFLOW.md`, and `.xcoding/KNOWLEDGE.md`, validate feature IDs, open the work order, fix its document language, and compute a validated plan. This path has no separate preparation step, so capture the baseline here: after opening the work order and before the first implementation leaf, determine and publish `work_order.report_baseline` (baseline commit, worktree digest, and digest algorithm) by running `xc-change-report`'s `scripts/capture_baseline.py`, whose `--format keys` output is the publishable form of exactly those three values. If that baseline cannot be determined before the first implementation leaf — for example the repository state is unreadable or the project is not a Git worktree — fail closed before the first mutating leaf instead of substituting the branch point or the first commit, and never rewrite a baseline that was captured late.
...
6. Build the dynamic sequence in this order when selected: goal, analysis or diagnosis, clarification, solution, approval, implementation units, verification scopes, change report, independent review, result, finalizer.
...
A mutation-mode plan carries the `change_report` capability when the resolved report policy commits to a report; read-only modes never acquire it from an escalation, even when unknown facts escalate to the full capability set. See the Change Report Policy section for how the tier is resolved and how a skip is recorded. On this path **the flat `role=report` node is the only supported form**: add one `report` leaf carrying the report contract, and do not embed the `xc-change-report` subtree, because the plan declares that leaf and the exactly-one-source rule cannot bind a subtree root — whose role is `root` and whose logical key is empty — to the plan's `report` role. Append that leaf and every other plan-required node **before the first leaf starts**, because a successful leaf seals the tree and a later append returns `tree_sealed`; "after the implementation and verification nodes" describes the leaf's sequence position in the dynamic group, never the moment it is added. The plan receipt's `report_strength` is the initial strength tier derived from the confirmed `risk` and `audit` facts, and the manifest measurement may still upgrade it upward, never downward. The main session publishes the two keys the report leaf is driven by: `report.strength` from the validated receipt's own `report_strength` field, which is already bound into `plan_id`, and `report.path` from the report leaf's terminal artifact once it exists. An unpublished tier is not an unset key: the adaptive tree declares `report.strength` at `standard`, the report specification's own declared default for that key, so a main session that never publishes the receipt's tier leaves the report leaf driven by that declared `standard` tier instead of by an absent value. That declared fallback is the contract — the tier is declared on both lifecycle trees with that same value, and no guard key is added for its publication — so the main session still publishes the receipt's `report_strength` whenever the plan carries the `change_report` capability. Name both keys in the report leaf's `metadata.control_packet.blackboard_keys` declaration when it must read them, because a dynamic leaf sees only the blackboard keys its own declaration projects. The adaptive tree declares `report.path` for exactly this reason: so an independent review leaf can read the report at all. The report-before-review order in this section is main-session policy with no runtime mechanism behind it — nothing compares the two leaves' completion order — so the report is the reviewer's input only when the main session sequences the leaves as stated. When the measured analyzable change unit count is zero, the report is not produced and the main session records `work_order.report_skip_reason=zero_analyzable_units` instead.
...
3. Call `xc-open-work-order`, initialize `assets/work-order-template.xml`, determine and validate `work_order.document_language` through `xc-document`, then complete `prepare-work-order`. An explicit `document_language` wins; otherwise use only the initiating request's clearly dominant language or `en`.
   ...
   In the same preparation step, write the commitment `work_order.requires_report`. Resolve it from the report policy below, not from the mode alone. Also record `work_order.report_baseline` once — baseline commit, worktree digest, and digest algorithm — by running `xc-change-report`'s `scripts/capture_baseline.py`, whose `--format keys` output is the publishable form of exactly those three values; recording them by hand satisfies this step's text while leaving the capture undone, and the report stage then fails with `baseline_record_missing`. Set the initial `report.strength` from the confirmed `risk` and `audit` facts: `risk=low` with `audit=runtime-only` is `minimal`; `risk=high`, `audit=result`, or `audit=full` is `full`; everything else is `standard`. All three are preparation-phase records, not retrospective notes. A late baseline lets earlier implementation changes escape the covered change set, and `work_order.requires_report` is a commitment, never a status bit: its declared default is `true`, so an unwritten commitment selects `report-group` instead of silently skipping the report, and the write this preparation step performs is what makes the commitment. Step 10 refuses to close a work order whose report obligation was neither satisfied nor explicitly skipped.
...
9. Add approved implementation nodes through `xc-implementation`, then verification nodes through `xc-verification` only after `approved-solution-continuation` is selected. Publish each node's declared source IDs through its node-specific blackboard key before requesting its control packet; do not substitute ancestor or group IDs.
   A work order that produces analyzable change units also produces a change report. `report-group` is the last group before `result-document`, mounted after `verification-group`, because only implemented and verified code has a state worth describing; and that full lifecycle mounts no top-level review node, so a review that the project bridge or an approved solution requires is extra work inside the implementation path and happens before the report rather than between the report and the result document. The other half of the same statement is the adaptive path, where the order is main-session policy rather than something the engine enforces: when it selects `independent_review`, the main session sequences the report leaf before that review leaf so the report is the reviewer's input, and a review that changes code refreshes the report once more afterwards.
10. Create `result.md` through document evolution. Publish the terminal goal writer ID in `work_order.objective_source_ids` and the terminal result writer ID in `work_order.result_source_ids`, read the finalizer control packet, and complete `finalize-work-order`.
    Consume `work_order.report_skip_reason`: when `work_order.requires_report=true` and neither a report path nor one of the legal skip reasons is recorded, the work order must not close. A silent skip is not a legal outcome. Record the report path, the manifest path, and the workbench path on one fixed line of the result document's report section; when the same delivery spans other work orders, list their report paths there too, because coverage is proven per work order and this file does not aggregate it.

## Change Report Policy

Whether a work order produces a change report is a default with an override, resolved once during preparation. Four inputs are consulted in a fixed order, and each later input may only be used when no earlier one decided:

1. **An explicit user instruction.** A user who asks for or waives the report decides it. This always wins.
2. **The project bridge.** A bridge may only tighten: it may turn a would-be skip into a required report, never the reverse. This matches the bridge rule used by `operation=classify`.
3. **The workshop default**, `report.default` in `.xcoding/xc-orchestration-runtime.json`.
4. **The built-in default**, used when the configuration has no `report` section.

`report.default` has three values:

- `never` — the report is not produced for any mode.
- `code-change-only` — the commitment follows the mode: `true` for `change`, `repair`, and `maintenance`; `false` for `investigation` and `review`; and `true` when the mode cannot be confirmed, because an unknown mode is never read as "no report needed". **This is the built-in default and reproduces the previous mode-derived behaviour exactly**, so adding the configuration section changes nothing for a project that does not set it.
- `always` — the commitment is `true` for every mode, including read-only ones.

Every skip records why. The legal values of `work_order.report_skip_reason` are `read_only_mode`, `zero_analyzable_units`, `user_waived`, and `policy_default`, the last for a skip the resolved tier produced. Extending the vocabulary is what keeps step 10's rule intact: a policy skip is now expressible, so it is recorded rather than silent, and a skip with no reason remains illegal.

`code-change-only` needs no separate change-set probe. A mutation-mode work order mounts the group, and a change set with no analysable units reaches the existing `zero_analyzable_units` outcome inside it.

Pace is not part of this resolution. `pace=fast` trims optional depth only; it never removes a capability, and using it to drop the report would break the planner's monotonicity invariant. A work order that should omit the report does so through the resolved tier, not through its pace.

Once covered code changes after a report was generated, the report is stale. Staleness is routed to the bounded refresh back-edge of `report-pass-loop` rather than to a node failure: increment `report.round`, write `report.refresh_reason` (`rework_rejected`, `rework_revision_required`, `stale_head`, or `review_driven_repair`; `initial` on the first round), increment `report.refresh_count` when code rework caused the round, and re-enter at `prepare-manifest`. Every round regenerates the same single `change-report.html`; the loop is bounded at three iterations, and an exhausted loop escalates to a blocked stage that a human resolves rather than being retried without limit or failing the work order. When the optional `report-gate` is enabled it re-opens in every round, so the gate never reads a stale report.
...
When a work order stops progressing, check once whether the obstruction is the
task or the tooling before spending more attempts on it. A task blocker is
missing information, an unavailable dependency, or an unresolved human
decision, and it is handled through a gate, recovery work, or an explicit
blocked node. A tool defect is the runtime or a workflow contract refusing an
operation the task itself permits, and its signature is a refusal that names no
reachable next action. For a tool defect, record how the tool obstructed the
work, use the supported bypass the runtime provides for that state, and say in
the result what was bypassed and why. When no bypass applies, close the work
order as failed or leave it open with the obstruction recorded; a documented
unfinished work order is a better outcome than silent abandonment or a
fabricated success. Keep this check short — it is a moment of triage, not an
investigation phase, and it never justifies bypassing a refusal that does name
a next action.
