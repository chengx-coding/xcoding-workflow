...
3. Call `xc-open-work-order`, initialize `assets/work-order-template.xml`, determine and validate `work_order.document_language` through `xc-document`, then complete `prepare-work-order`. An explicit `document_language` wins; otherwise use only the initiating request's clearly dominant language or `en`.
   ...
   In the same preparation step, write the mode-derived commitment `work_order.requires_report`: `true` for `change`, `repair`, and `maintenance`; `false` plus `work_order.report_skip_reason=read_only_mode` for `investigation` and `review`; and `true` when the mode cannot be confirmed, because an unknown mode is never interpreted as "no report needed". Also record `work_order.report_baseline` once — baseline commit, worktree digest, and digest algorithm — and set the initial `report.strength` from the confirmed `risk` and `audit` facts: `risk=low` with `audit=runtime-only` is `minimal`; `risk=high`, `audit=result`, or `audit=full` is `full`; everything else is `standard`. All three are preparation-phase records, not retrospective notes. A late baseline lets earlier implementation changes escape the covered change set, and `work_order.requires_report` is a commitment, never a status bit: its declared default is `true`, so an unwritten commitment selects `report-group` instead of silently skipping the report, and the mode-derived write this preparation step performs is what makes the commitment. Step 10 refuses to close a work order whose report obligation was neither satisfied nor explicitly skipped.
