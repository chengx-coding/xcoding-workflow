...
The root has exactly these fields: `schema_version`, `profile_id`, `owner_skill`, `description`, `instruction_resources`, `required_skills`, `capability_vocabulary`, `capabilities`, `context`, `outputs`, `delegation`, `failure_policy`, and `compatibility`.
...
Host, model, sandbox, permission, runtime tree, node, gate, retry, loop, project topology, and project command fields are not profile fields. Unknown fields fail validation.
