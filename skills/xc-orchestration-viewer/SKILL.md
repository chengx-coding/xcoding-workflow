...
Explicit `--tree` paths are automatically allowed. Direct path registration
from the page is limited to explicit `--allow-root` paths. The native file
...
- Save SVG downloads a complete standalone tree, including nodes currently collapsed in the browser presentation.
